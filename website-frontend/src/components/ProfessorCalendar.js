...
class ProfessorCalendar extends Component {
  ...
  handleShowAdd = ({ start, end }) =>
    this.setState({ start: start, end: end, showAdd: true });
  handleShowEdit = ({ _id, start, end }) =>
    this.setState({ eventid: _id, start: start, end: end, showEdit: true });
    ...
    if (this.state.start && this.state.end && this.state.newevent) {
      ...
      try {
        const res = await axios.post(
          "http://localhost:8080/calendar/addevent",
          data
        );
  ...
  handleEdit = async () => {
    if (
      ...
      try {
        const res = await axios.post(
          "http://localhost:8080/calendar/updateevent",
          data
        );
        res.data.results.start = new Date(res.data.results.start);
        res.data.results.end = new Date(res.data.results.end);
        let index = this.state.eventslist.findIndex(
          (element) => element._id === this.state.eventid
        );
  ...
  handleDelete = async () => {
    if (this.state.eventid) {
      ...
      try {
        const res = await axios.post(
          "http://localhost:8080/calendar/deleteevent",
          data
        );
        let index = this.state.eventslist.findIndex(
          (element) => element._id === this.state.eventid
